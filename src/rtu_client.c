...
RTU_CL_RTU_STATE rtu_cl_rtu_state;
uint8_t rtu_cl_crcErrors;

void rtu_cl_init() {
    // RS485 already in receive mode
    rtu_cl_rtu_state = RTU_CL_RTU_IDLE;
    rtu_cl_crcErrors = 0;
}

// Called often
__bit rtu_cl_poll() {
    if (rs485_isMarkCondition && rtu_cl_rtu_state != RTU_CL_RTU_IDLE) {
        if (rtu_cl_rtu_state != RTU_CL_RTU_WAIT_FOR_RESPONSE) {
            // Abort reading, go idle
            rtu_cl_rtu_state = RTU_CL_RTU_IDLE;
            return false;
        } else {
            rtu_cl_rtu_state = RTU_CL_RTU_RESPONSE;
        }
    }

    if (rtu_cl_rtu_state == RTU_CL_RTU_IDLE) {
        ...
        if (rtu_cl_header.header.stationAddress == STATION_NODE) {
            if (rtu_cl_header.header.function == READ_HOLDING_REGISTERS || rtu_cl_header.header.function == WRITE_HOLDING_REGISTERS) {
                if (!regs_validateReg()) {
                    ...
                    rtu_cl_rtu_state = RTU_CL_RTU_WAIT_FOR_RESPONSE;
                    return false;
                }
                ...
                if (rtu_cl_header.header.function == READ_HOLDING_REGISTERS) {
                    ...
                    // response
                    rtu_cl_rtu_state = RTU_CL_RTU_CHECK_REQUEST_CRC;
                } else {
                    rtu_cl_rtu_state = RTU_CL_RTU_RECEIVE_DATA_SIZE;
                }
            } else {
            ...
            rtu_cl_rtu_state = RTU_CL_RTU_WAIT_FOR_IDLE;
        }
    }
    
    if (rtu_cl_rtu_state == RTU_CL_RTU_RECEIVE_DATA_SIZE) {
        ...
        if (rs485_buffer[0] != messageSize) {
            ...
            rtu_cl_rtu_state = RTU_CL_RTU_RECEIVE_DATA;
        }
    }

    if (rtu_cl_rtu_state == RTU_CL_RTU_RECEIVE_DATA) {
    ...
    if (rtu_cl_rtu_state == RTU_CL_RTU_CHECK_REQUEST_CRC) {
        // CRC is LSB first
    ...
    if (rtu_cl_rtu_state == RTU_CL_RTU_WAIT_FOR_IDLE) {
        rs485_discard(rs485_readAvail());
        return false;
    }

    if (rtu_cl_rtu_state == RTU_CL_RTU_RESPONSE) {
        ...
        *((ModbusRtuHoldingRegisterRequest*)rs485_buffer) = rtu_cl_header;
        if (rtu_cl_exceptionCode == NO_ERROR) {
            ...
            if (rtu_cl_header.header.function == READ_HOLDING_REGISTERS) {
                ...
                rtu_cl_rtu_state = RTU_CL_RTU_SEND_DATA;
            } else {
                rs485_write(sizeof(ModbusRtuPacketWriteResponse));
                rtu_cl_rtu_state = RTU_CL_RTU_WRITE_RESPONSE_CRC;
            }
        } else {
    ...
    if (rtu_cl_rtu_state == RTU_CL_RTU_SEND_DATA) {
    ...
    if (rtu_cl_rtu_state == RTU_CL_RTU_WRITE_RESPONSE_CRC) {
        if (rs485_writeInProgress()) {
            return false;
        }
        // CRC is LSB first
        *((uint16_t*)rs485_buffer) = htole16(crc16);
        ...
        rtu_cl_rtu_state = RTU_CL_RTU_WAIT_FOR_FLUSH;
        return false;
    }

    if (rtu_cl_rtu_state == RTU_CL_RTU_WAIT_FOR_FLUSH) {
